class NumberFlip {
  ...
  public setNumber(num: number, animate = true): void {
  ...
  private adjustAmountOfDigitContainers(num: number): void {
    ...
    let countOfDigitContainers = this.rootElement.getElementsByClassName(
    ...
    if (countOfDigitContainers > numberOfDigits) {
      if (!this.rootElement) return;
      const digitContainers = this.rootElement.getElementsByClassName(
        this.digitClassname
      );
      ...
      for (let i = numberOfDigits; i < digitContainers.length; i++) {
        ...
        if (!!digitContainerParent) {
          setTimeout(
            () => digitContainerParent?.removeChild(digitContainer),
            this.durationFade
          );
        }
      }
  ...
  private calculateTranslateY(digit: number) {
    ...
    // console.log(document.getElementsByClassName("numberflip-digit-container")[0].offsetHeight)
